...
function isActivePage(activePage: string, page: string) {
  if (page === "home") {
          ...
          <div className="flex flex-row items-center gap-4">
            {pages.map((page) => (
              <Button
                asChild
                className={cn(
                  "p-0 font-normal text-muted-foreground hover:text-foreground hover:no-underline",
                  isActivePage(pathname, page.href) && "text-foreground"
                )}
                key={page.href}
                variant="link"
              >
      ...
      <div className="hidden lg:flex">
        <div className="hidden flex-row items-center justify-end gap-2 px-6 md:px-4 lg:col-span-1 lg:col-start-2 xl:flex">
          ...
          <Button aria-label="GitHub" asChild size="icon-sm" variant="outline">
            <Link
              href="https://github.com/vercel/ai-feedback"
              target="_noblank"
            >
              <LogoGithub />
            </Link>
