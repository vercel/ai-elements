...
You now have a working chatbot app with file attachment support! The chatbot can handle both text and file inputs through the action menu. Feel free to explore other components like [`Tool`](/apps/docs/content/docs/components/(chatbot)/tool.mdx) or [`Task`](/apps/docs/content/docs/components/(chatbot)/task.mdx) to extend your app, or view the other examples.
