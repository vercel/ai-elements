...
Render the stack trace collapsed initially:
...
### With File Path Click Handler

Handle clicks on file paths to open in editor:
...
<StackTrace
  ...
  onFilePathClick={(path, line, column) => {
    ...
    window.open(`vscode://file/${path}:${line}:${column}`);
  }}
>
  {/* ... */}
</StackTrace>
