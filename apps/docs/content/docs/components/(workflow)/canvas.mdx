...
<Callout>
  The Canvas component is designed to be used with the [Node](elements/components/node) and [Edge](elements/components/edge) components. See the [Workflow](/examples/workflow) demo for a full example.
</Callout>
